...
for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
    ...
    for contour in c: 
        ...
        if k<50:
            cv2.imwrite("/home/pi/Test/Face/image%04i.jpg" %k,img)
            k +=1
            
            original=cv2.imread("/home/pi/Test/Captured/image%04i.jpg" %(j-1))
            duplicate=cv2.imread("/home/pi/Test/Face/image%04i.jpg" %(k-1))
            ...
            if original.shape == duplicate.shape:
                ...
                if cv2.countNonZero(b) == 0 and cv2.countNonZero(g) == 0 and cv2.countNonZero(r) == 0:
                    ...
                    cv2.imwrite("/home/pi/Test/diff/image%04i.jpg" %l,duplicate)
                    l+=1
                    #cv2.imshow("Original", original)
                    cv2.imshow("Duplicate", duplicate)
                    camera.start_preview()
                    camera.start_recording("/home/pi/Test/diff/video%04i.h264" %l)
                    sleep(10)
